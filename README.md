...
האלגוריתמים הטהורים והטיפוסים שלהם נמצאים ב־`src/financial-agents.ts`. כל מנוע מממש `FinancialAgentStrategy`, והמחלקה `FinancialAgentsOrchestrator` מקבלת את האסטרטגיות ב־dependency injection ומחזירה תוצאה טיפוסית אחת; `runFinancialAgents` הוא Facade תואם לשכבת התצוגה. שכבת התצוגה והאישור נמצאת ב־`src/app.ts`. כלל שנלמד נשמר במערך `rules`; תקרת תקציב שאושרה נשמרת ב־`budgets`. הממצאים והזדמנויות החיסכון נגזרים מחדש ואינם נשמרים כמצב נוסף.
