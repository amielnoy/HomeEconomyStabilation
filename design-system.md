...
- The summary line is a control (`cardgroup-toggle`): it opens the charges in place as ordinary transaction rows, keeps `aria-expanded` honest, shows a focus ring, and is touch-sized on coarse pointers.
- It carries a `title` saying what the click does — «לחיצה תפרוס כאן את כל 22 החיובים של ויזה» — and offers the way back once open. The line reads as a total and gives nothing else away: the caret is a hint and `aria-expanded` speaks only to a screen reader, so without it a household has no way to know the charges are one click off. A card's sum with no way through to its charges would hide the detail rather than fold it.
