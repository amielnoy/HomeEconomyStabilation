...
| `tests/e2e/card-summary-view.e2e.spec.ts` | The transactions table opening on statement rows with each card folded to the sum it charged, opening a card onto the charges behind it, itemising every charge on request, a card report without a statement staying itemised, the count and totals reading the same in both views, and a row's dot carrying the direction of its money rather than the category's chart slot |
