...
| `tests/unit/bank-importer.unit.test.ts` | Hebrew and English bank headers, account extraction, card-source signed amounts, recognising a card report by its columns rather than by the control it arrived through, transaction normalization and stable IDs, and a statement that names its operation column הפעולה beside an empty פרטים being read with a description the rules can act on |
...
| `tests/unit/import-corrections.unit.test.ts` | Matching a row saved from the old reading to the row that replaces it by replaying that reading over the same file: an installment whose billed amount differs from the purchase amount, a foreign purchase whose amount and reference were both read from the wrong columns, and a purchase waived in full having no billed row to keep |
...
| `tests/server/test_models.py` | Pydantic allowlists, size bounds, financial-identifier rejection, the card issuer, a categorisation rule carrying the direction it reads end to end through a snapshot, and a savings goal crossing the boundary with its name checked, its shape refused when unknown and its absence accepted from a browser that never had one, and the four digits that name a card crossing the boundary, with anything that is not exactly four digits refused |
...
| `tests/e2e/quick-add.e2e.spec.ts` | Recording a cash expense from the button on the screen, the chosen category outranking the rule that would have claimed the description, the dialog opening on the amount with today filled in, only categories money can leave through being offered, a dismissed dialog adding nothing, the same expense refused a second time, an amount that is not money refused, and the path offered in every language, the plus button leading to the expenses screen with the cursor on the amount, the form emptying after a save, what was recorded there listed and removable, and nothing a statement brought in listed beside it, the month's spending so far, a ring per category with a limit carrying both figures, a passed limit said in words rather than drawn only, and a household with no limits told where to set them |
...
| `tests/e2e/screen-navigation.e2e.spec.ts` | Every screen named without opening a menu, the screen the reader is on marked and only that one, moving between screens and back to the dashboard, the bar staying one line on a phone without pushing the page sideways, and the screens offered in every language, the months named in the transactions filter and on the credit-cards screen, choosing one moving the whole dashboard with it, and all history staying with the table that asked for it |
| `tests/e2e/card-statements.e2e.spec.ts` | The credit-cards screen: every charge listed with the number of cards it came from, one card at a time when chosen from the combo box, the cards named and sorted with every card first, each card printed newest first, every charge written as a red minus, an honest message with no card report imported, the dashboard hidden and given back, and the screen offered in every language, each card listed on its own in the chooser and folded on its own in the transactions table, a card nobody identified still known by its issuer, and a pasted card number keeping nothing while the import proceeds and says so |
| `tests/e2e/card-summary-view.e2e.spec.ts` | The transactions table opening on statement rows with each card folded to the sum it charged, opening a card onto the charges behind it, itemising every charge on request, a card report without a statement staying itemised, the count and totals reading the same in both views, and a folded card filed as a credit-card bill whichever issuer it came from while its charges keep their own categories, and the folded line saying what clicking it will do, in every language and in both directions |
...
| `tests/e2e/settlement-detail.e2e.spec.ts` | Opening a card settlement onto the charges it paid for, a total equal to the line it opened from, the month totals left alone when a bill from another month is opened, a bill whose detail was never imported saying so, and no control offered where no card report exists |
| `tests/e2e/transaction-direction.e2e.spec.ts` | Saying one row is spending and having the whole row read as it, taking it back, the answer surviving a reload, a chosen category carried across, the control naming the row it is about, and every other row left where it was |
...
| `tests/e2e/reimport-correction.e2e.spec.ts` | A row saved the wrong way round being replaced rather than duplicated when the file is read correctly, the count of corrected rows said out loud, a category the customer chose carried across to the row that replaces it, and a refund the statement itself reported left alone, and a legacy purchase total replaced by the billed installment, kept across a second import and a reload |
...
- `tests/e2e/discovery.e2e.spec.ts`: public Hebrew/English guides with JavaScript disabled, RTL/LTR, responsive layout, 48px controls and WCAG checks in light/dark modes.
- Discovery contracts also verify reciprocal guide language links, canonical URLs, structured-data consistency, deployment inclusion and API exclusions in each crawler group.
