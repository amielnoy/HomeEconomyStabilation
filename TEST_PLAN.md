...
| `tests/server/test_models.py` | Pydantic allowlists, size bounds, financial-identifier rejection, the card issuer, a categorisation rule carrying the direction it reads end to end through a snapshot, and a savings goal crossing the boundary with its name checked, its shape refused when unknown and its absence accepted from a browser that never had one, and the name the household gave a card crossing the boundary with the same identifier check a description meets |
