...
`goals`, a collection beside `budgets`, holds what a household is saving towards: a name it typed, a target, how much it has put aside and an optional target month. Both figures are the household's own — nothing infers them from transactions, because no statement says which transfer belonged to which goal. The name is free text, so it is redacted on the way out exactly as a description is and the boundary refuses one that still reads as an account or card number; the collection is capped at 100 goals, and the target month is a `YYYY-MM` string that carries no day.

`cardLast4`, on a transaction, holds the last four digits of the card a report came from, typed at import beside the issuer question that was already asked there. It exists because two cards from one issuer are two cards and no export says which is which; four digits are what every receipt prints and what a household recognises its own card by. The shape is the protection: the field accepts exactly four digits and nothing else at every boundary, so it cannot hold a card number, an account number or an IBAN however they are typed. A longer value — a pasted card number — is not truncated into something that looks like an answer: the import proceeds without it, keeps nothing of it, and says so. It never reaches a log line; that a card was identified is recorded, the digits are not.
