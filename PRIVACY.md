...
`cardName`, on a transaction, is what the household calls the card a report came from — typed at import, beside the issuer question that was already asked there. It exists because two cards from one issuer are two cards and no export says which is which: the alternative to a name the customer chose is a card number, which this boundary refuses. It is free text, so it is redacted on the way out exactly as a description is, capped at 40 characters, and rejected by both boundaries if it still reads as an account or card number. It never reaches a log line — that a card was named is recorded, what it was called is not.
