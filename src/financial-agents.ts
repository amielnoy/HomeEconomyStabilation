...
export interface PaydayRunway { balance: number; nextIncomeDate: string | null; committed: number; freeToSpend: number }
...
export function paydayAgent(transactions: AgentTransaction[]): PaydayRunway | null {
  ...
  if (!recurringIncome) return { balance: latest.bal!, nextIncomeDate: null, committed: 0, freeToSpend: latest.bal! };
  ...
  if (nextIncome.getTime() <= latestTime) nextIncome = new Date(Date.UTC(cursor.getUTCFullYear(), cursor.getUTCMonth() + 1, expectedDay));
  const committed = [...groups(transactions.filter((item) => item.out > 0)).values()]
    ...
    .reduce((sum, items) => {
      ...
      if (due.getTime() <= latestTime) due.setUTCMonth(due.getUTCMonth() + 1);
