...
describe('TypeScript source contract', () => {
  it('keeps project-owned executable source out of JavaScript files', () => {
    ...
    const ownedSources = [...sourceFiles('fe/src'), ...sourceFiles('scripts'), ...sourceFiles('tests'), ...rootFiles];
  ...
  it('loads all project-owned browser behavior from compiled TypeScript modules', () => {
    for (const file of ['mazan-habait.html', 'Architecture.html', 'api-docs.html', 'scalar-docs.html']) {
      const html = readFileSync(resolve(root, 'fe', file), 'utf8');
    ...
    expect(readFileSync(resolve(root, 'fe/Architecture.html'), 'utf8')).toContain('dist/architecture.js');
    expect(readFileSync(resolve(root, 'fe/api-docs.html'), 'utf8')).toContain('/dist/api-docs.js');
    expect(readFileSync(resolve(root, 'fe/scalar-docs.html'), 'utf8')).toContain('/dist/scalar-docs.js');
