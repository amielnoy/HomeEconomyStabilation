...
export const cleanTransactionText = (value) => String(value ?? '').replace(BIDI, '').replace(/\s+/g, ' ').trim();
...
const findHeader = (rows) => {
    for (let rowIndex = 0; rowIndex < Math.min(rows.length, 30); rowIndex += 1) {
        ...
        if (map.date !== undefined && map.desc !== undefined
            && (map.out !== undefined || map.in !== undefined || map.amt !== undefined))
            return { row: rowIndex, map };
    }
    return null;
};
...
export class BankImportStrategy {
    import(workbook, filename, source = 'bank') {
        const found = [];
        let account = null;
            ...
            for (let rowIndex = 0; rowIndex < Math.min(rows.length, 12) && !account; rowIndex += 1) {
                ...
                for (let column = 0; column < row.length; column += 1) {
                    ...
                    if (cell?.t === 's' && /^חשבון/.test(cleanTransactionText(cell.v))) {
                        ...
                        if (next)
                            account = cleanTransactionText(next.v);
                    }
                }
            }
